...
describe('loadContentDirectory', () => {
  ...
  it('validates the runtime content that ships with the repository', () => {
    ...
    expect(kinds.filter((kind) => kind === 'fishing-spots')).toHaveLength(147)
    ...
    expect(kinds.filter((kind) => kind === 'gear')).toHaveLength(470)
    ...
    expect(kinds.filter((kind) => kind === 'contacts')).toHaveLength(5)
    expect(kinds.filter((kind) => kind === 'species-trade-profiles')).toHaveLength(229)
    expect(kinds.filter((kind) => kind === 'contact-rewards')).toHaveLength(80)
  })
})
